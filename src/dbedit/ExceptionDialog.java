...
public class ExceptionDialog {
    ...
    public static void showException(Throwable t) {
        ...
        if (t instanceof SQLException) {
            ...
            if (t.getMessage().indexOf("Unsupported VM encoding") != -1) {
            ...
            } else if (CustomAction.connectionData != null && CustomAction.connectionData.isIbm()) {
                if (t.getMessage().indexOf("JDBC 2 method called: Method is not yet supported") != -1) {
                    ...
                    int length = CustomAction.columnTypes.length;
                    for (int i = 0; i < length; i++) {
                        if (CustomAction.isLob(i)) {
                            msg = "The IBM JDBC driver doesn't support modifying rows with BLOB's or CLOB's in the resultset yet.\n" +
